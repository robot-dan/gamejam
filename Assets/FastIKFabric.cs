...
/*        if ((targetPosition - GetPositionRootSpace(Bones[0])).sqrMagnitude >= (CompleteLength * CompleteLength))
        {
            Debug.Log("longer than length");
            float targetDistance = (targetPosition - GetPositionRootSpace(Bones[0])).magnitude;
            for (int i =0; i < BonesLength.Length; i++)
            {
                BonesLength[i] =  targetDistance/ BonesLength.Length;
            }
            Debug.Log(targetDistance / BonesLength.Length);
